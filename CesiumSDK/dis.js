...
MeasureUtils.delentity = function() {
  if (tmpmeasurelineEntity) viewer.entities.remove(tmpmeasurelineEntity)
  if (tmpmeasurepolygonEntity) viewer.entities.remove(tmpmeasurepolygonEntity)
  if (measurepolygonEntity) viewer.entities.remove(measurepolygonEntity)
  if (measurelineEntity) viewer.entities.remove(measurelineEntity)
  // if(measurearealabel)
  //	viewer.entities.remove(measurearealabel);
  for (let i = 0; i < measurelineEntityArray.length; i++) {
  ...
  for (let i = 0; i < measurepointEntityArray.length; i++) {
    viewer.entities.remove(measurepointEntityArray[i])
  }
  for (let i = 0; i < measurepolygonEntityArray.length; i++) {
    viewer.entities.remove(measurepolygonEntityArray[i])
  }
  for (let i = 0; i < measurelabels.length; i++) {
    viewer.entities.remove(measurelabels[i])
  }
  boolmeasure = false
  measuredistance = 0
  measurepointsarr = []
  ...
  measurearealabels2.removeAll()
  viewer.scene.globe.depthTestAgainstTerrain = false
  jQuery('#measureTitle').html(
    '右键单击结束，再次单击继续或新建执行。双击删除。'
  )
  jQuery('#measureResult').html('')
}
...
MeasureUtils.caldistance = function(
  startLongitude,
  startLatitude,
  startHeight,
  endLongitude,
  ...
  var cartographic1 = Cesium.Cartographic.fromDegrees(
    startLongitude,
    startLatitude,
    startHeight
  )
  var cartesian1 = ellipsoid.cartographicToCartesian(cartographic1)
  var cartographic2 = Cesium.Cartographic.fromDegrees(
    endLongitude,
    endLatitude,
    endHeight
  )
...
MeasureUtils.ringArea = function(coords) {
  ...
  if (coordsLength > 2) {
    for (let i = 0; i < coordsLength; i++) {
      ...
      area +=
        (MeasureUtils.rad(p3[0]) - MeasureUtils.rad(p1[0])) *
        Math.sin(MeasureUtils.rad(p2[1]))
    }
...
MeasureUtils.stickground = function(x0, y0, z0, x1, y1, z1, boolline) {
  ...
  for (var i = 0; i < nsplit; i++) {
    ...
    cartographic = new Cesium.Cartographic(
      lonarr[i] * Math.PI / 180,
      latarr[i] * Math.PI / 180,
      0
    )
